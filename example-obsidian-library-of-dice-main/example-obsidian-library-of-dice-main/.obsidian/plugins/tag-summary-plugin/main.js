...
var SummaryPlugin = class extends import_obsidian3.Plugin {
    ...
    return __async(this, null, function* () {
      ...
      this.registerMarkdownCodeBlockProcessor("add-summary", (source, el, ctx) => __async(this, null, function* () {
        ...
        rows.forEach((line) => {
          ...
          if (line.match(/^\s*includeNote:[\p{L}0-9_\-/# ]+$/gu)) {
            const content = line.replace(/^\s*includeNote:/, "").trim();
            let list = content.split(/\s+/).map((tag) => tag.trim());
            list = list.filter((tag) => {
              if (tag.match(/^#[\p{L}]+[^#]*$/u)) {
                return true;
        ...
        if (tags.length > 0 || include.length > 0 || includeNote.length > 0) {
          yield this.createSummary(el, tags, include, includeNote, exclude, ctx.sourcePath);
        } else {
  ...
  createSummary(element, tags, include, includeNote, exclude, filePath) {
    return __async(this, null, function* () {
      var _a;
      ...
      listFiles = listFiles.filter((file) => {
        ...
        const matchesIncludeNote = includeNote.length == 0 ? true : includeNote.every((value) => tagsInFile.includes(value));
        const matchesValidTags = validTags.length == 0 ? true : validTags.some((value) => tagsInFile.includes(value));
        return matchesIncludeNote && matchesValidTags;
      });
      ...
      if (summary != "") {
        ...
        yield import_obsidian3.MarkdownRenderer.renderMarkdown(summary, summaryContainer, (_a = this.app.workspace.getActiveFile()) == null ? void 0 : _a.path, this);
