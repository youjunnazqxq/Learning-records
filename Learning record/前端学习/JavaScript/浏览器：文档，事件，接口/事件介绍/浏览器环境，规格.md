...
***事件（Event）是浏览器对“刚刚发生了某件事”发出的一个通知（Notification）或信号（Signal）
...
处理程序中的 `this` 的值是对应的元素。就是处理程序所在的那个元素，就是指代的是我们点击的那个DOM元素
...
```
<button onclick = 'alert("啊都对赌")'>
```

## 通过在js中对象中的属性添加 

```
btm = document.getElementById('myBotton');
btn = function(){
	alert("n hao ");
}
```

## 通过addEventListener（）方法添加 
